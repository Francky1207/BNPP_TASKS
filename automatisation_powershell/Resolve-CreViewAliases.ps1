    ...
    if (-not $NoExcelOutput) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $total; $i++) {
                ...
                # Cast [string] systematique : evite l'InvalidCastException de
                # l'adaptateur COM lorsque des types differents se succedent.
                $ws.Cells.Item($lx, 1).Value2 = [string]$row.CRE
                $ws.Cells.Item($lx, 2).Value2 = [string]$row.TABLE
                $ws.Cells.Item($lx, 3).Value2 = [string]$row.COLONNES
                $ws.Cells.Item($lx, 4).Value2 = [string]$row.VIEW_NAME
                $ws.Cells.Item($lx, 5).Value2 = [string]$row.TABLE_TFT
                $ws.Cells.Item($lx, 6).Value2 = [string]$row.ALIAS
                $ws.Cells.Item($lx, 7).Value2 = [string]$row.EXPRESSION_SOURCE
                $ws.Cells.Item($lx, 8).Value2 = [string]$row.STATUT
