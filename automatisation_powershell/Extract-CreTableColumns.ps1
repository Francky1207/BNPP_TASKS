...
try {
    ...
    if ($null -eq $ws) {
        # Repli automatique : recherche parmi toutes les feuilles celle qui
        # contient les entetes requis (CRE + REQUEST), plutot que d'exiger
        # un nom de feuille exact. Le nom reel varie selon les classeurs
        # ("CRE", "1-CRE", etc.) : cette detection evite une fragilite inutile.
        foreach ($s in $wb.Worksheets) {
            $urTest = $s.UsedRange
            $dTest = $urTest.Value2
            if ($dTest -isnot [System.Array]) { continue }

            $rTest = [int]$dTest.GetLowerBound(0)
            $cLoTest = [int]$dTest.GetLowerBound(1)
            $cHiTest = [int]$dTest.GetUpperBound(1)
            $aCre = $false; $aRequest = $false
            for ($c = $cLoTest; $c -le $cHiTest; $c++) {
                $val = Get-NormalizedText $dTest.GetValue($rTest, $c)
                if ($val -eq 'cre') { $aCre = $true }
                if ($val -eq 'request') { $aRequest = $true }
            }
            if ($aCre -and $aRequest) { $ws = $s; break }
        }
    }
    if ($null -eq $ws) {
        $dispo = New-Object 'System.Collections.Generic.List[string]'
        foreach ($s in $wb.Worksheets) { $dispo.Add($s.Name) }
        $nomDemande = if ($SourceSheet) { $SourceSheet } else { '(non precise)' }
        throw ("Aucune feuille contenant les entetes CRE + REQUEST trouvee (nom demande : {0}). Feuilles disponibles : {1}" -f
               $nomDemande, ($dispo -join ', '))
    }
    Write-Verbose ("Feuille source utilisee : '{0}'" -f $ws.Name)

    $ur   = $ws.UsedRange
    $data = $ur.Value2                                   # lecture en bloc = 1 appel COM
    if ($data -isnot [System.Array]) { throw "Feuille '$($ws.Name)' vide." }
    ...
    if ($colCre -eq 0 -or $colRequest -eq 0) {
        throw "Entetes 'CRE' et/ou 'REQUEST' introuvables sur la 1ere ligne de la feuille '$($ws.Name)'."
    }
    ...
    # exit (et non throw) : un "throw" non rattrape produit un second message
    # d'erreur natif PowerShell sur le flux stderr, qui perturbe la capture
    # de sortie des outils externes (harnais de test, orchestrateurs...).
    # Le diagnostic a deja ete affiche ci-dessus ; on sort simplement avec
    # un code d'erreur.
    exit 1
}
finally {
